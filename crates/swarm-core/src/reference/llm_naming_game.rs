//! # 💡 [参考答案区] LLM Naming Game 标准参考实现 (Reference Implementation)
//!
...
/// LLM Naming Game 仿真系统标准参考实现
#[derive(Clone, Debug)]
pub struct LlmNamingGameReference<G: Network> {
    pub network: G,
...
impl<G: Network> LlmNamingGameReference<G> {
...
mod tests {
    ...
    #[test]
    fn test_ref_llm_rates() {
        let (pi_llama, phi_llama) = LlmArchitecture::Llama3_1_8B.get_rates(1.0);
        assert!((pi_llama - 0.775).abs() < 1e-4);
        assert!((phi_llama - 0.275).abs() < 1e-4);

        let (pi_mistral, phi_mistral) = LlmArchitecture::Mistral7B.get_rates(0.0);
        assert_eq!(pi_mistral, 0.99);
        assert!((phi_mistral - 0.02).abs() < 1e-4);
    }

    #[test]
    fn test_ref_llm_step_and_consensus() {
        let mut rng = StdRng::seed_from_u64(42);
        let graph = CompleteGraph::new(4);
        let mut game = LlmNamingGameReference::new(
            graph,
        ...
        assert_eq!(game.distinct_words(), 1);
    }
}
