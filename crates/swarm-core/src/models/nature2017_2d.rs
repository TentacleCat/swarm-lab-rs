...
/// 2D Swarmalator model from O'Keeffe, Hong & Strogatz (Nature Communications 2017)
/// "Oscillators that sync and swarm"
#[derive(Clone, Debug)]
pub struct Swarmalator2D {
    pub n: usize,
    /// Spatial attraction/repulsion coupling influenced by phase correlation
    pub j: f64,
    /// Phase coupling strength
    pub k: f64,
    /// Natural frequencies of oscillators
    pub omega: Vec<f64>,
    /// Softening parameter to prevent singularity at dist -> 0
    pub eps: f64,
}
...
impl Swarmalator2D {
    pub fn new(n: usize, j: f64, k: f64) -> Self {
        Self {
            ...
            j,
            k,
            omega: vec![0.0; n], // Identical oscillators by default
            eps: 1e-5,
        }
    }
    ...
    /// x, y ~ Uniform(-1, 1), theta ~ Uniform(-pi, pi)
    pub fn random_initial_state<R: Rng>(&self, rng: &mut R) -> Vec<f64> {
        ...
        for i in 0..self.n {
            state[i] = pos_dist.sample(rng);             // x_i
            state[self.n + i] = pos_dist.sample(rng);     // y_i
            state[2 * self.n + i] = phase_dist.sample(rng); // theta_i
        }
        state
    }
}
    ...
    fn derivative(&self, s: &[f64], ds: &mut [f64]) {
        ...
        (0..n).into_par_iter().map(|i| {
            ...
            for j in 0..n {
                ...
                // Spatial force: (dx_ij / dist) * (1 + J*cos(dth)) - (dx_ij / dist_sq)
